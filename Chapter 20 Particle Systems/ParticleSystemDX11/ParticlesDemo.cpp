...
bool ParticlesApp::Init()
{
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, flares.data()->c_str(), &texResource, &mFlareTexSRV));
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, raindrops.data()->c_str(), &texResource, &mRainTexSRV));
	ReleaseCOM(texResource); // view saves reference
